...
class UserControllerClass implements ControllerInterface {
	...
	public function doAction()
	{
		...
		switch ( $this->getAction() )
        {
			...
			default:
				$errors = array();
				$outPutData[0]= false;
				$errors[]= "Sorry, there has been an error. Try later(User controller)";
				$outPutData[]= $errors;
	...
	private function entryUser()
	{
		...
		$user = new User();
		$user->setAll(0, $userObj->name, $userObj->surname1, $userObj->nick, $userObj->password,$userObj->rol, $userObj->address, $userObj->telephone, $userObj->mail, $userObj->birthDate, date("Y-m-d h:i:sa"), "0000-00-00", $userObj->active, $userObj->image);
